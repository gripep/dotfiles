...
# Ask for the administrator password upfront (abort if it is declined, so we
# don't apply a half-configured mix of user and privileged settings)
if ! sudo -v; then
    echo "Administrator authentication is required to apply macOS defaults. Aborting."
    exit 1
fi
